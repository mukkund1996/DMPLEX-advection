static char help[] = "Simple Advection-diffusion equation with a non-linear term solved using FVM in DMPLEX\n";

/*
   Solves the simple advection equation given by

   q_t + u (q_x) + v (q_y) - D (q_xx + q_yy) + C exp(q) = 0 using FVM and First Order Upwind discretization. 
...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options) {
    ...
    CHKERRQ(ierr);
    ierr = PetscOptionsString("-filename", "The mesh file", "advection_DMPLEX.c", options->filename, options->filename,
    ...
    CHKERRQ(ierr);
    ierr = PetscOptionsScalar("-u", "The x component of the convective coefficient", "advection_DMPLEX.c", options->u,
                              &options->u, NULL);
    CHKERRQ(ierr);
    ierr = PetscOptionsScalar("-v", "The y component of the convective coefficient", "advection_DMPLEX.c", options->v,
                              &options->v, NULL);
    CHKERRQ(ierr);
    ierr = PetscOptionsScalar("-diffus", "The diffusive coefficient", "advection_DMPLEX.c", options->diffusion,
                              &options->diffusion, NULL);
    CHKERRQ(ierr);
    ierr = PetscOptionsScalar("-c", "The Non-linear coefficient", "advection_DMPLEX.c", options->C, &options->C, NULL);
    CHKERRQ(ierr);
...
static PetscErrorCode CreateMesh(MPI_Comm comm, AppCtx *user, DM *dm) {
    ...
    if (!len) {
        ...
        ierr = DMPlexCreateBoxMesh(comm, user->dim, PETSC_FALSE, user->cells, NULL, NULL, NULL, user->interpolate, dm);
        CHKERRQ(ierr);
...
PetscErrorCode FormFunction(TS ts, PetscReal ftime, Vec X, Vec F, void *ctx) {
    ...
    /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ierr = DMPlexGetHeightStratum(da, 0, &cStart, &cEnd);
    CHKERRQ(ierr);
...
/*TEST
    ...
    test:
      suffix: 2
      args: -ts_max_steps 5  -u 1.0 -v 0.0 -diffus 1.0 -C 0.01 -draw_pause 10

    test:
      suffix: 3
      args: -ts_max_steps 5 -ts_type glle -u 1.0 -v 0.0 -diffus 1.0 -C 0.01 -draw_pause 10
