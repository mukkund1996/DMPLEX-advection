...
static PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  ierr = PetscOptionsScalar("-c", "The Non-linear coefficient", "advection_DMPLEX.c", options->C, &options->C, NULL);CHKERRQ(ierr);
...
PetscErrorCode FormFunction(TS ts,PetscReal ftime,Vec X,Vec F,void *ctx)
{
  ...
  for (cell = cStart; cell < cEnd; cell++) {
    ...
    flux_centre = x[cell] * ((u_plus - u_minus + 2 * user->diffusion * delta_y)/delta_x + (v_plus - v_minus + 2 * user->diffusion * delta_x)/delta_y) - PetscExpScalar(x[cell]) * delta_x * delta_y;
