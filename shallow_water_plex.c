...
int main(int argc, char **argv) {
    ...
    CHKERRQ(ierr);
    ierr = DMPlexCreateSection(da, NULL, numComp, numDof, numBC, bcField, NULL, bcPointIS, NULL, &user.sect);
    CHKERRQ(ierr);
    ierr = ISDestroy(&bcPointIS[0]);
    CHKERRQ(ierr);

    /* Name the Field variables */
    ierr = PetscSectionSetFieldName(user.sect, 0, "h");
    CHKERRQ(ierr);
    ierr = PetscSectionSetFieldName(user.sect, 0, "uh");
    CHKERRQ(ierr);

    /* Tell the DM to use this data layout */
    ierr = DMSetLocalSection(da, user.sect);
    CHKERRQ(ierr);
    user.da = da;
    ...
    - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ierr = FormInitialSolution(da, x, user.sect);
    CHKERRQ(ierr);
...
PetscErrorCode FormFunction(TS ts, PetscReal ftime, Vec X, Vec F, void *ctx){
   ...
   - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    for (cell = cStart; cell < cEnd; cell++) {
        ...
        /* Obtaining the offset of the cell component of the corresponding fields */
        ...
        for (field = 0; field < 2; field++)
            /* Obtaining the offset of the neighboring cells of the corresponding fields */
        ...
        /* Computing the net fluxes in each cell and defining the array f (q = dq/dt) */
