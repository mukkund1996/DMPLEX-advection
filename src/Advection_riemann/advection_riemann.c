...
PetscErrorCode SetInitialCondition(DM dm, Vec X, User user) {
    ...
    ierr = DMProjectFunction(dm, 0.0, func, ctx, INSERT_ALL_VALUES, X);
    CHKERRQ(ierr);
    PetscFunctionReturn(0);
}
...
int main(int argc, char **argv) {
    ...
    CHKERRQ(ierr);
    ierr = DMViewFromOptions(dm, NULL, "-orig_dm_view");
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = SetInitialCondition(dm, X, user);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = PetscPrintf(PETSC_COMM_WORLD, "%s at time %g after %D steps\n", TSConvergedReasons[reason], (double) ftime,
                       nsteps);
    CHKERRQ(ierr);
